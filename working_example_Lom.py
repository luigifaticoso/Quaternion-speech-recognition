...
# # Training
# #
# path = "./drive/My Drive/COLAB Neural/logging"
...
#
path = "./drive/My Drive/COLAB Neural/logging"
cbk = keras.callbacks.TensorBoard(log_dir=path, histogram_freq=0,  
...
# Save Path
classifier.save_weights(path + "/saved_model.h5")
print("Saved model to disk")
...
# path = "./logging"
# cbk = TensorBoard(path)

# #filepath = os.path.join(path, "saved-model-{epoch:02d}-{val_acc:.2f}.hdf5")
# #checkpoint = ModelCheckpoint(filepath, monitor='val_acc', save_best_only=True, save_weights_only=False, mode='auto', period=1)
